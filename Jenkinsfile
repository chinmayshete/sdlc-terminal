pipeline {
    ...
    parameters {
        ...
        string(name: 'VERSION_TAG', defaultValue: '', description: 'Release version tag')
        booleanParam(name: 'RUN_SECURITY_SCAN', defaultValue: true, description: 'Run OWASP + secret scanner')
        booleanParam(name: 'RUN_SONAR', defaultValue: false, description: 'Run SonarQube analysis')
        booleanParam(name: 'SKIP_TESTS', defaultValue: false, description: 'Skip unit tests')
        booleanParam(name: 'FORCE_ROLLBACK', defaultValue: false, description: 'Roll back ECS task')
    }
    ...
    stages {
        stage('Checkout') {
            steps {
                checkout scm
                script {
                    env.GIT_TAG_NAME = sh(script: "git describe --tags --exact-match 2>/dev/null || echo ''", returnStdout: true).trim()
        ...
        stage('Lint & Standards') {
            parallel {
                stage('TypeScript Compile') { steps { sh 'npx tsc --noEmit' } }
                stage('ESLint') {
                    steps {
        ...
        stage('SonarQube Analysis') {
            ...
            steps {
                withSonarQubeEnv('SonarQube-Server') {
                    sh "npx sonar-scanner -Dsonar.projectKey=${SONAR_PROJECT_KEY} -Dsonar.host.url=${SONAR_HOST_URL} -Dsonar.token=${SONAR_TOKEN}"
                }
    ...
    post {
        ...
        success {
            script { notifySlack("✅ Build #${env.BUILD_NUMBER} Succeeded", 'good') }
        }
        failure {
            script { notifySlack("🚨 Build #${env.BUILD_NUMBER} Failed", 'danger') }
        }
    }
}
...
def deployToECS(Map args) {
    ...
    def appName = 'sdlc-terminal'

    echo "Deploying to ECS ${cluster}/${service}..."
...
def notifySlack(String message, String color) {
    sh "curl -s -X POST ${env.SLACK_WEBHOOK} -H 'Content-Type: application/json' -d '{\"attachments\":[{\"color\":\"${color}\",\"text\":\"${message}\"}]}'"
}
