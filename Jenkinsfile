...
pipeline {
    ...
    environment {
        ...
        GITHUB_CREDENTIALS  = credentials('github-app-credentials')
    ...
    stages {
        ...
        stage('Vault Auth') {
            steps {
                script {
                    // AppRole login — exchange Role ID + Secret ID for a short-lived token
                    ...
                    // Fetch Azure OpenAI key from Vault
                    env.AZURE_OPENAI_API_KEY = sh(
                        script: """
                            curl -s -H "X-Vault-Token: ${env.VAULT_TOKEN}" \
                                ${VAULT_ADDR}/v1/secret/data/sdlc/azure \
                            | python3 -c "import sys,json; d=json.load(sys.stdin); print(d['data']['data']['AZURE_OPENAI_API_KEY'])"
                        """,
                        returnStdout: true
                    ).trim()

                    echo "Vault auth successful. Secrets loaded."
                }
            }
        }
