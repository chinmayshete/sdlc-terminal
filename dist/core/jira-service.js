...
class JiraService {
    constructor() {
        this.baseUrl = env_1.env.jiraHost ? `https://${env_1.env.jiraHost.replace(/\/+$/, "")}/rest/api/3` : "";
        this.auth = (env_1.env.jiraEmail && env_1.env.jiraApiToken)
            ? Buffer.from(`${env_1.env.jiraEmail}:${env_1.env.jiraApiToken}`).toString("base64")
            : null;
    }
    isConfigured() {
    ...
    async fetchTickets() {
        ...
        try {
            const jql = `project = "${env_1.env.jiraProjectKey}" AND statusCategory != Done ORDER BY created DESC`;
