...
async function generateCode(ticket, files) {
    ...
    try {
        const response = await callAzureJson([
            {
                role: "system",
                content: 'You are a TypeScript code generation assistant for a local Express repo. Return JSON only with shape {"files": [{"path": string, "content": string}]}. Always return complete file contents. Every file path must stay inside repo/app/. Never return src/, tests/, or any path outside repo/app/.',
            },
            {
                role: "user",
                content: JSON.stringify({
...
async function generateTests(ticket, codeChanges) {
...
async function generateFreeNlpChat(files, history, prompt) {
...
async function explainFileWithChat(filePath, content, history) {
    ...
    try {
        const response = await callAzureJson([
            {
                role: "system",
                content: 'You explain code clearly for a developer in a terminal. Return JSON only with shape {"message": string}. Be practical, concise, and specific to the provided file.',
            },
            {
                role: "user",
                content: JSON.stringify({
...
async function checkAiHealth() {
    const configured = (0, env_1.hasAzureOpenAiConfig)();
    const mode = env_1.env.useMock ? "mock" : "azure";
    if (!configured || env_1.env.useMock) {
        return {
            configured,
            mode,
            reachable: true,
...
function shouldUseAzure() {
    return !env_1.env.useMock && (0, env_1.hasAzureOpenAiConfig)();
}
...
function normalizeCodeResponse(files, fallback) {
    if (!files || files.length === 0) {
        return fallback;
    }
    return files.filter((file) => typeof file.path === "string" && typeof file.content === "string");
}
function buildFallbackNlpChat(files, prompt) {
    const lowerPrompt = prompt.toLowerCase();
    const targetedEdit = parseTargetedEdit(prompt);
    if (isGreeting(lowerPrompt)) {
        return {
            message: "Hey. I'm in NLP mode with access to the local repo, so you can chat normally, ask questions, or tell me to modify files.",
            changes: [],
        };
    }
    if (targetedEdit) {
        const targetFile = files.find((file) => normalizePath(file.path) === normalizePath(targetedEdit.path));
        if (!targetFile) {
            return {
            ...
            changes: [
                {
                    ...
                    content: applyBasicTargetedEdit(targetFile.content, targetedEdit.instruction),
                },
            ],
        };
    }
    if (lowerPrompt.includes("what files") ||
    ...
    if (looksLikeCodeChangeRequest(lowerPrompt)) {
        ...
        return {
            message: "I applied a code-oriented interpretation of your request to the local repo.",
            changes: buildFallbackCode(syntheticTicket, files),
        };
    }
    return {
        message: "I'm ready. Ask me about the codebase, request a refactor, create a file, explain a module, summarize the app, or describe a feature you want changed.",
        changes: [],
    };
}
function isGreeting(prompt) {
    return ["hi", "hello", "hey", "yo"].some((word) => prompt === word || prompt.startsWith(`${word} `));
}
function looksLikeCodeChangeRequest(prompt) {
    const verbs = [
        "add",
        "update",
        "modify",
...
function parseTargetedEdit(prompt) {
...
function applyBasicTargetedEdit(content, instruction) {
    return `${content.trimEnd()}\n\n// NLP note: ${instruction}\n`;
}
function normalizePath(value) {
    return value.replace(/\\/g, "/").toLowerCase();
}
function buildFallbackPlan(ticket, files) {
    const names = files
...
function buildFallbackCode(ticket, files) {
...
function buildFallbackTests(ticket) {
    const lowerText = `${ticket.title} ${ticket.description}`.toLowerCase();
    if (lowerText.includes("register") || lowerText.includes("signup")) {
        return [
            {
...
function buildLoginChanges(files) {
...
function buildRegisterChanges(files) {
    return buildBaseAuthRepo(files, {
        includeLoginRoute: true,
...
function buildProfileChanges(files) {
    return buildBaseAuthRepo(files, {
        includeLoginRoute: true,
        includeRegisterRoute: true,
...
function buildBaseAuthRepo(files, options) {
...
`;
    ...
    if (options.includeLoginRoute) {
        routeLines.push('router.post("/login", async (request, response) => {', "  try {", "    const result = await login(request.body);", "    response.status(200).json(result);", "  } catch (error) {", '    const message = error instanceof Error ? error.message : "Unknown error";', "    response.status(401).json({ error: message });", "  }", "});", "");
    }
    if (options.includeRegisterRoute) {
        routeLines.push('router.post("/register", async (request, response) => {', "  try {", "    const result = await register(request.body);", "    response.status(201).json(result);", "  } catch (error) {", '    const message = error instanceof Error ? error.message : "Unknown error";', "    response.status(400).json({ error: message });", "  }", "});", "");
    }
    routeLines.push('router.get("/users", async (_request, response) => {', "  const users = await listUsers();", "  response.status(200).json(users);", "});", "");
    if (options.includeProfileRoute) {
        routeLines.push('router.get("/me", async (request, response) => {', '  const email = String(request.query.email ?? "demo@example.com");', "", "  try {", "    const profile = await getCurrentUserProfile(email);", "    response.status(200).json(profile);", "  } catch (error) {", '    const message = error instanceof Error ? error.message : "Unknown error";', "    response.status(404).json({ error: message });", "  }", "});", "");
    }
    ...
    return [
        ...
        },
        {
            ...
            content: byPath.get("repo/app/src/server.ts") ??
                `import express from "express";
